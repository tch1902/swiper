...
def add_swipe_score(swipe_type,sid):
    ...
    update_swipe_score(score, sid)
    print(rds.zrevrange('hot_rank',0,2,withscores=True))

def sub_swipe_score(swipe_type,sid):
    '''
    为滑动操作减少积分
    :param swipe_type:
    :param sid:
...
def update_swipe_score(score,sid):
    '''
    为用户更新积分
    :param score:
    :param sid:
...
def get_top_rank(num):
    ...
    #单个获取
    # rank_data=[{'user':User.get(pk=uid).to_dict(),'score':score} for uid,score in clearn_data]
